...
const MedicalRecordUpload = () => {
  ...
  return (
    <Box sx={{ paddingTop: "2rem", width: "60%", margin: "0 auto" }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', flexWrap: 'wrap', marginTop: '1rem' }}>
        <Box sx={{ width: "calc(50% - 1rem)", marginBottom: "1rem" }}>
          <TextField name="patientId" variant="outlined" fullWidth value={formData.patientId} onChange={handleInputChange} label="Patient ID" />
          {errors.patientId && <Typography variant="body2" color="error">{errors.patientId}</Typography>}
        </Box>
        <Box sx={{ width: "calc(50% - 1rem)", marginBottom: "1rem" }}>
          <TextField name="patientName" variant="outlined" fullWidth value={formData.patientName} onChange={handleInputChange} label="Patient Name" />
          {errors.patientName && <Typography variant="body2" color="error">{errors.patientName}</Typography>}
        </Box>
        <Box sx={{ width: "calc(50% - 1rem)", marginBottom: "1rem" }}>
          <TextField name="age" type="number" variant="outlined" fullWidth value={formData.age} onChange={handleInputChange} label="Age" />
          {errors.age && <Typography variant="body2" color="error">{errors.age}</Typography>}
        </Box>
        <Box sx={{ width: "calc(50% - 1rem)", marginBottom: "1rem" }}>
          <TextField name="gender" variant="outlined" fullWidth value={formData.gender} onChange={handleInputChange} label="Gender" />
          {errors.gender && <Typography variant="body2" color="error">{errors.gender}</Typography>}
        </Box>
        <Box sx={{ width: "calc(50% - 1rem)", marginBottom: "1rem" }}>
          <TextField name="bloodGroup" variant="outlined" fullWidth value={formData.bloodGroup} onChange={handleInputChange} label="Blood Group" />
        ...
        <Box sx={{ width: "100%", marginBottom: "1rem" }}>
          ...
          <Box sx={{ position: 'relative' }}>
            ...
            <input type="file" accept=".pdf" onChange={handleFileUpload} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', opacity: 0, cursor: 'pointer' }} />
            <CloudUploadOutlinedIcon sx={{ position: 'absolute', top: '50%', right: '0.5rem', transform: 'translateY(-50%)', cursor: 'pointer' }} />
          </Box>
          {errors.medicalRecordFile && <Typography variant="body2" color="error">{errors.medicalRecordFile}</Typography>}
        </Box>
        <Box sx={{ width: "100%", marginBottom: "1rem" }}>
          <Typography variant="body1" sx={{ marginBottom: "0.5rem" }}>Description (Optional):</Typography>
          <TextField name="description" variant="outlined" multiline rows={4} fullWidth value={formData.description} onChange={handleInputChange} />
        </Box>
        <FormControlLabel
