...
const NavBar = ({role}) => {
    ...
    return (
        <Stack direction="row" justifyContent="space-between" alignItems="center" padding={2} width="100%">
            <img width={200} src={logo} alt="" flexGrow={1}
                 style={{marginRight: "auto"}}/> {/* Adjust logo width and add margin-right */}
            <Stack direction="row" spacing={2} style={{marginLeft: "auto"}}> {/* Adjust spacing and add margin-left */}
                {filteredNavLinks.map((link, index) => (
                    <Link to={link.link} key={index} onClick={() => handleLinkClick(link)}
                          style={{textDecoration: "none"}}>
                        <Typography variant="body2" sx={{
                            fontWeight: selectedLink === link ? "bold" : "normal",
                            color: selectedLink === link ? "#424242" : "#757575",
                            padding: "8px",
                            borderRadius: "4px",
